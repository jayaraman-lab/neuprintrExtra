...
supertype.character <- function(types,level=2,unicodeDelta=TRUE){
  ...
  supertype[is.na(supertype)] <- "Unassigned"
  supertype[! supertype %in% c("PB Interneurons","FB Columnar","EB Columnar","ER","ExR","FB Output","FB Tangential","FB Interneuron","DN","MBON","DAN","5HT","OA","Peptidergic","Clock","Fru","Other Sensory","LH","Visual PNs","Antennal lobe","Unassigned")] <- "Terra incognita"
