...
get_type2typePath_raw <- function(type.from=NULL,
  ...
  for (n in downHalf){
    ...
    if(!is.null(thresholdPerROI)){bag$outputs <- filter(bag$outputs,(totalROIweight*n_targets/n_type)>thresholdPerROI & 
                                                                     totalPreROIweight>thresholdPerROI)}
  ...
  for (n in rev(upHalf)){
    ...
    if(!is.null(thresholdPerROI)){bag$inputs <- filter(bag$inputs,(totalROIweight*n_targets/n_type)>thresholdPerROI  & totalPreROIweight>thresholdPerROI)}
